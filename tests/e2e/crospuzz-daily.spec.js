...
const SVC_URL = "https://appadaycreator.github.io/crospuzz-daily/";
const SVC_NAME = "クロスワードパズル";

test.describe("クロスワードパズル - E2Eテスト", () => {

  test("ページ基本: タイトル・h1・canonical", async ({ page }) => {
    await page.goto(SVC_URL);
    const title = await page.title();
    expect(title.length).toBeGreaterThan(15);
    expect(title.toLowerCase()).toMatch(/クロスワード/i);
    const canonical = await page.locator('link[rel="canonical"]').getAttribute("href");
    expect(canonical).toContain("crospuzz-daily");
  ...
  test("SEO: sitemap.xml・robots.txt が存在する", async ({ page }) => {
    const sitemapResp = await page.request.get("https://appadaycreator.github.io/crospuzz-daily/sitemap.xml");
    expect([200, 301, 302]).toContain(sitemapResp.status());
    const robotsResp = await page.request.get("https://appadaycreator.github.io/crospuzz-daily/robots.txt");
  ...
  test("プライバシー・利用規約: ページが存在する", async ({ page }) => {
    const privResp = await page.request.get("https://appadaycreator.github.io/crospuzz-daily/privacy-policy.html");
    const termsResp = await page.request.get("https://appadaycreator.github.io/crospuzz-daily/terms.html");
